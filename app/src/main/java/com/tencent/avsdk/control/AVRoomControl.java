...
class AVRoomControl {
	...
	private AVRoomPair.Delegate mRoomDelegate = new AVRoomPair.Delegate() {
		...
		protected void OnRoomCreateComplete(int result) {
			...
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_ROOM_CREATE_COMPLETE)
					.putExtra(AVUtil.EXTRA_ROOM_ID, mRoomId).putExtra(
							AVUtil.EXTRA_AV_ERROR_RESULT, result));
		}
		...
		protected void OnRoomJoinComplete(int result) {
			...
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_ROOM_JOIN_COMPLETE)
		...
		protected void OnRoomPeerLeave() {
			...
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_PEER_LEAVE));
		}
		
		protected void OnCameraStart() 
		{
			Log.d(TAG, "OnCameraStart"  );	
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_PEER_CAMERA_OPEN));
		}
		protected void OnCameraClose() 
		{
			Log.d(TAG, "OnCameraClose"  );	
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_PEER_CAMERA_CLOSE));
		}		
		protected void OnMicStart()
		{
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_PEER_MIC_OPEN));
			Log.d(TAG, "OnMicStart"  );	
		}
		protected void OnMicClose()
		{
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_PEER_MIC_CLOSE));
...
//			mContext.sendBroadcast(new Intent(AVUtil.ACTION_PEER_LEAVE));
	...
	private AVContext.CloseRoomCompleteCallback mCloseRoomCompleteCallback = new AVContext.CloseRoomCompleteCallback() {
		protected void OnComplete() {
			...
			mContext.sendBroadcast(new Intent(AVUtil.ACTION_CLOSE_ROOM_COMPLETE));
		}
	};
